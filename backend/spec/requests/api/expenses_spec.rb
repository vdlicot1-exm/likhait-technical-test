...
RSpec.describe "Api::Expenses", type: :request do
  ...
  describe "POST /api/expenses" do
    ...
    context "with invalid parameters" do
      it "rejects expenses with negative amounts" do
        invalid_params = {
          expense: {
      ...
      it "rejects expenses with empty descriptions" do
        invalid_params = {
          expense: {
            description: "",
            amount: 100.00,
    ...
    context "with a future date" do
      it "rejects expenses dated in the future" do
        ...
        expect(response).to have_http_status(:unprocessable_entity)
        json = JSON.parse(response.body)
        expect(json["errors"]).to include(a_string_matching(/cannot be in the future/))
      end
    end
  end
end
